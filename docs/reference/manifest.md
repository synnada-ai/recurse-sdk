...
JSON Schema (Draft 2020-12) describing run inputs. `task` is a reserved string property and must either have a non-empty default or be required from the caller. Supplied values are validated against the schema and property defaults are applied. The resolved `task` string, templated with `{{ input.NAME }}` placeholders, becomes the agent task and is excluded from `recurse.context().inputs`.
